  ...
  return (
    <TooltipProvider>
      ...
      >
            ...
            {admin && (
              <Tooltip>
                <TooltipTrigger asChild>
                  <span className="inline-flex items-center bg-amber-100 text-amber-900 px-2 py-0.5 rounded text-xs ml-2 font-semibold cursor-pointer">
                    Admin
                    <Info className="ml-1 w-3 h-3 text-amber-700" />
                  </span>
                </TooltipTrigger>
                <TooltipContent side="top" className="max-w-xs">
                  <span>Du bist ein Administrator dieses Portals.<br />
                  Admins erhalten Zugriff auf exklusive Bereiche wie das Admin-Dashboard.</span>
                </TooltipContent>
              </Tooltip>
            )}
          </div>
        ...
        {/* Custom Role */}
        <div>
          <Label htmlFor="custom_role">
            Deine selbstgewählte Rolle{" "}
            {form.custom_role && (
              <span className="ml-2 text-green-700 bg-green-100 px-2 rounded text-xs">{form.custom_role}</span>
            )}
          </Label>
          <Input
            name="custom_role"
          ...
          {form.custom_role && (
            <div className="text-xs text-muted-foreground mt-1">
              Dieser Text wird z.B. in Kommentaren angezeigt.
            </div>
          )}
        </div>
        {/* Premium Member */}
